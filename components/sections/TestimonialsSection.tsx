...
export default function TestimonialsSection() {
  ...
  const next = () => setCurrent((prev) => (prev + 1) % Math.ceil(testimonials.length / visible));
  const prev = () => setCurrent((prev) => (prev - 1 + Math.ceil(testimonials.length / visible)) % Math.ceil(testimonials.length / visible));

  const displayed = testimonials.slice(current * visible, current * visible + visible);

  return (
    <section className="py-20 lg:py-28 bg-brand-slate/5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <AnimatePresence mode="wait">
              {displayed.map((t) => (
                ...
                >
                  <Quote size={32} className="text-brand-gold/40 mb-4" />
